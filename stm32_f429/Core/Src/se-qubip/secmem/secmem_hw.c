...
secmem_alg_t secmem_aes = { .alg_id = "AES", .max_keys = KEY_NUM_AES, .key_len =
		KEY_LEN_AES, .addr_offset = KEY_OFFSET_AES, .num_keys_stored = 0,
		.key_slot_in_use = { false } };

secmem_alg_t secmem_x25519 = { .alg_id = "X25519", .max_keys = KEY_NUM_X25519,
		.key_len = KEY_LEN_X25519, .addr_offset = KEY_OFFSET_X25519,
		.num_keys_stored = 0, .key_slot_in_use = { false } };

secmem_alg_t secmem_eddsa = { .alg_id = "EDDSA", .max_keys = KEY_NUM_EDDSA,
		.key_len = KEY_LEN_EDDSA, .addr_offset = KEY_OFFSET_EDDSA,
		.num_keys_stored = 0, .key_slot_in_use = { false } };

secmem_alg_t secmem_slhdsa = { .alg_id = "SLHDSA", .max_keys = KEY_NUM_SLHDSA,
		.key_len = KEY_LEN_SLHDSA, .addr_offset = KEY_OFFSET_SLHDSA,
		.num_keys_stored = 0, .key_slot_in_use = { false } };

secmem_alg_t secmem_mlkem = { .alg_id = "MLKEM", .max_keys = KEY_NUM_MLKEM,
		.key_len = KEY_LEN_MLKEM, .addr_offset = KEY_OFFSET_MLKEM,
		.num_keys_stored = 0, .key_slot_in_use = { false } };

secmem_alg_t secmem_mldsa = { .alg_id = "MLDSA", .max_keys = KEY_NUM_MLDSA,
		.key_len = KEY_LEN_MLDSA, .addr_offset = KEY_OFFSET_MLDSA,
		.num_keys_stored = 0, .key_slot_in_use = { false } };

secmem_alg_t *alg_info[] = { &secmem_aes, &secmem_x25519, &secmem_eddsa,
		&secmem_slhdsa, &secmem_mlkem, &secmem_mldsa };
...
void secmem_store_key(uint8_t alg_id, uint8_t *key_id, bool is_external,
	...
	uint16_t secmem_op_sel = ((uint16_t) alg_id << ALG_ID_OFFSET)
			| ((uint16_t) is_external << KEY_ORIGIN_OFFSET) | OP_STORE_KEY;
...
void secmem_delete_key(uint8_t alg_id, uint8_t key_id, INTF interface) {
	...
	const uint16_t secmem_code = SECMEM_SE_CODE;
	uint16_t secmem_op_sel = ((uint16_t) alg_id << ALG_ID_OFFSET)
			| ((uint16_t) key_id << KEY_ID_OFFSET) | OP_DELETE_KEY;
...
void secmem_get_key(uint8_t alg_id, uint8_t key_id, uint8_t *key_data,
	...
	const uint16_t secmem_code = SECMEM_SE_CODE;
	uint16_t secmem_op_sel = ((uint16_t) alg_id << ALG_ID_OFFSET)
			| ((uint16_t) (key_id) << KEY_ID_OFFSET) | OP_GET_KEY;
...
void secmem_info(int DBG, INTF interface) {
	...
	for (uint32_t i = 0; i < num_alg; i++) {
		...
		printf("\n %-8s|   Keys: %2u / %-2u  |  Size: %4u bytes", alg_name,
				keys_in_use, max_keys, key_len);
		...
		for (uint32_t i = 0; i < num_alg; i++) {
			...
			for (uint8_t key_id = 0; key_id < max_keys; key_id++) {
				...
				if (is_in_use) {
					...
					printf(" %-8s|   Key ID: %2u     | Value:  ", alg_name,
							key_id);
