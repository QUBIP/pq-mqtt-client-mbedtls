...
void read_I2C(I2C_FD i2c_fd, void *data, size_t offset, size_t size_data) {
	...
	//-- Write Pointer Index
	HAL_I2C_Master_Transmit(&hi2c1, I2C_SLAVE_ADDRESS, &ptr_idx, 1,
	I2C_TIMEOUT);
	//-- Read from I2C Port
	HAL_I2C_Master_Receive(&hi2c1, I2C_SLAVE_ADDRESS, data, size_data,
			I2C_TIMEOUT);
}
...
void write_I2C(I2C_FD i2c_fd, void *data, size_t offset, size_t size_data) {
	...
	HAL_I2C_Master_Transmit(&hi2c1, I2C_SLAVE_ADDRESS, buf, 1 + size_data,
	I2C_TIMEOUT);
...
void read_I2C_ull(I2C_FD i2c_fd, void *data, size_t offset, size_t size_data) {
	...
	HAL_I2C_Master_Receive(&hi2c1, I2C_SLAVE_ADDRESS, data_char, size_data,
	I2C_TIMEOUT);
	//-- Cast char to unsigned long long
	size_t size_data_ull =
			(size_data % 8 == 0) ? (size_data / 8) : (size_data / 8 + 1);
