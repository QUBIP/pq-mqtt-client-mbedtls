...
/* Stack task size*/
static StackType_t modbusClientTask_stack[MODBUS_CLIENT_TASK_STACK_SIZE
		/ sizeof(StackType_t)];
/* Task Control Block */
...
__attribute__((section(".ccmram")))   volatile uint8_t ccmram_heap[CCRAM_HEAP_SIZE] =
		{ 0 };

const HeapRegion_t xHeapRegions[] = {
// Point to ccmram
//{ ( uint8_t * ) 0x10000000UL, 0xFFFE },
		{ (uint8_t*) &ccmram_heap, CCRAM_HEAP_SIZE }, { (uint8_t*) &heap,
		RAM_REGION_HEAP_SIZE }, { NULL, 0 } /* Terminates the array. */
};
...
void vApplicationStackOverflowHook(xTaskHandle xTask, signed char *pcTaskName) {
	...
	FREERTOS_DEBUG_LOG(
			"[FREERTOS] ERROR: freeHeapSize: %u bytes, minimumEverFreeHeapSize: %u bytes\r\n",
			(unsigned int )freeHeapSize,
			(unsigned int )minimumEverFreeHeapSize);
	while (1) {
...
void vApplicationGetRandomHeapCanary( portPOINTER_SIZE_TYPE *pxHeapCanary) {
	...
	if (HAL_RNG_GenerateRandomNumber(&hrng, &randomValue) != HAL_OK) {
		randomValue = ((HAL_GetTick() * HAL_GetUIDw0()) + HAL_GetUIDw1())
				^ HAL_GetUIDw2();
		FREERTOS_DEBUG_LOG(
				"[FREERTOS] ERROR: RNG failed, manual rng num randomValue: %u\r\n",
...
void StartDefaultTask(void const *argument) {
	...
	osThreadDef(mqttClientPubTask, MqttClientPubTask, osPriorityNormal, 0,
			(50 * 1024) / sizeof(StackType_t));
	mqttClientPubTaskHandle = osThreadCreate(osThread(mqttClientPubTask), NULL);
	if (mqttClientPubTaskHandle == NULL) {
		FREERTOS_DEBUG_LOG(
				"[FREERTOS] ERROR: Failed to create mqttClientPub Task\n");
		while (1) {
