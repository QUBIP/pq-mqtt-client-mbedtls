...
{
    uint8_t buf[KYBER_ETA1_512 * KYBER_N / 4];
...
{
    uint8_t buf[KYBER_ETA1_768 * KYBER_N / 4];
    prf(buf, sizeof(buf), seed, nonce);
    poly_cbd_eta1_768(r, buf);
}

void poly_getnoise_eta1_1024(poly* r, const uint8_t seed[KYBER_SYMBYTES], uint8_t nonce)
{
    uint8_t buf[KYBER_ETA1_1024 * KYBER_N / 4];
    prf(buf, sizeof(buf), seed, nonce);
    poly_cbd_eta1_1024(r, buf);
}
