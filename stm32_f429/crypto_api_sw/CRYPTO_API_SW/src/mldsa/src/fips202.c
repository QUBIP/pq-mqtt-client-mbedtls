...
void shake128_mldsa(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
  ...
  shake128_absorb_once_mldsa(&state, in, inlen);
  nblocks = outlen/SHAKE128_RATE;
  shake128_squeezeblocks_mldsa(out, nblocks, &state);
  outlen -= nblocks*SHAKE128_RATE;
  out += nblocks*SHAKE128_RATE;
  shake128_squeeze_mldsa(out, outlen, &state);
}
...
void shake256_mldsa(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
  ...
  shake256_absorb_once_mldsa(&state, in, inlen);
  nblocks = outlen / SHAKE256_RATE;
  shake256_squeezeblocks_mldsa(out, nblocks, &state);
  outlen -= nblocks*SHAKE256_RATE;
  out += nblocks*SHAKE256_RATE;
  shake256_squeeze_mldsa(out, outlen, &state);
}
