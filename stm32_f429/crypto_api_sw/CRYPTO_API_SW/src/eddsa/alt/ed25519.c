...
void ed25519GenerateKeyPair(const PrngAlgo *prngAlgo, void *prngContext, uint8_t *privateKey, uint8_t *publicKey)
{

   //Generate a private key
   ed25519GeneratePrivateKey(prngAlgo, prngContext, privateKey);
   ed25519GeneratePublicKey(privateKey, publicKey);
...
void ed25519GeneratePrivateKey(const PrngAlgo *prngAlgo, void *prngContext, uint8_t *privateKey)
{

   //The private key is 32 octets of cryptographically secure random data
   prngAlgo->read(prngContext, privateKey, ED25519_PRIVATE_KEY_LEN);
...
void ed25519GeneratePublicKey(const uint8_t *privateKey, uint8_t *publicKey)
{
   uint8_t *s;
...
void ed25519GenerateSignature(const uint8_t *privateKey,
   ...
   const void *context, uint8_t contextLen, uint8_t flag, uint8_t *signature)
{
   ...
   ed25519GenerateSignatureEx(privateKey, publicKey, messageChunks,
...
void ed25519GenerateSignatureEx(const uint8_t *privateKey,
   ...
   const void *context, uint8_t contextLen, uint8_t flag, uint8_t *signature)
{
   ...
   memcpy(state->s, state->sha512Context.digest, 32);
   ...
   memcpy(state->p, state->sha512Context.digest + 32, 32);
   ...
   const uint8_t *signature, uint8_t *ver)
{
   ...
   ed25519VerifySignatureEx(publicKey, messageChunks, context,
      contextLen, flag, signature, ver);
...
void ed25519VerifySignatureEx(const uint8_t *publicKey,
   ...
   uint8_t contextLen, uint8_t flag, const uint8_t *signature, uint8_t *ver)
{
   ...
   //half as a point R
   memcpy(state->r, signature, ED25519_SIGNATURE_LEN / 2);

   //Decode the second half as an integer S, in the range 0 <= s < L
   memcpy(state->s, signature + ED25519_SIGNATURE_LEN / 2,
