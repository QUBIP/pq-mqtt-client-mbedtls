   ...
   uint8_t *privateKey, uint8_t *publicKey)
{
   ...
   ed448GeneratePrivateKey(prngAlgo, prngContext, privateKey);

   ed448GeneratePublicKey(privateKey, publicKey);
}
   ...
   uint8_t *privateKey)
{
   ...
   prngAlgo->read(prngContext, privateKey, ED448_PRIVATE_KEY_LEN);
...
void ed448GeneratePublicKey(const uint8_t *privateKey, uint8_t *publicKey)
{
   ...
   //Erase working state
   memset(state, 0, sizeof(Ed448State));
   ...
   const void *context, uint8_t contextLen, uint8_t flag, uint8_t *signature)
{
   ...
   //Ed448 signature generation
    ed448GenerateSignatureEx(privateKey, publicKey, messageChunks,
   ...
   const uint8_t *signature, uint8_t *ver)
{
   ...
   ed448VerifySignatureEx(publicKey, messageChunks, context,
   ...
   uint8_t contextLen, uint8_t flag, const uint8_t *signature, uint8_t *ver)
{
   ...
   //half as a point R
   memcpy(state->r, signature, ED448_SIGNATURE_LEN / 2);

   //Decode the second half as an integer S, in the range 0 <= s < L
   memcpy(state->s, signature + ED448_SIGNATURE_LEN / 2,
