...
void shake128_slhdsa(uint8_t* out, size_t outlen, const uint8_t* in, size_t inlen)
{
    ...
    shake128_absorb_slhdsa(&state, in, inlen);
    shake128_squeezeblocks_slhdsa(out, nblocks, &state);

    out += nblocks * SHAKE128_RATE;
    outlen -= nblocks * SHAKE128_RATE;

    if (outlen) {
        shake128_squeezeblocks_slhdsa(t, 1, &state);
        for (i = 0; i < outlen; i++)
            out[i] = t[i];
    }
}
...
void shake256_slhdsa(uint8_t* out, size_t outlen, const uint8_t* in, size_t inlen)
{
    ...
    shake256_absorb_slhdsa(&state, in, inlen);
    shake256_squeezeblocks_slhdsa(out, nblocks, &state);

    out += nblocks * SHAKE256_RATE;
    outlen -= nblocks * SHAKE256_RATE;

    if (outlen) {
        shake256_squeezeblocks_slhdsa(t, 1, &state);
        for (i = 0; i < outlen; i++)
            out[i] = t[i];
    }
}
...
void sha3_512_slhdsa(uint8_t* h, const uint8_t* in, size_t inlen)
{
    unsigned int i;
    uint64_t s[25];
