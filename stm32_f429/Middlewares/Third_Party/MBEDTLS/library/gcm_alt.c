...
int __custom_set_key(mbedtls_gcm_context *ctx,
                       const unsigned char *key,
                       unsigned int keybits){

	ctx->keysize = keybits;
	memcpy(ctx->keyval,key,ctx->keysize / 8);
                       ...
                       unsigned int keybits)
{
    ...
    if(cipher == MBEDTLS_CIPHER_ID_AES) {
    	__custom_set_key(ctx, key, keybits);
    }
...
int custom_gcm_crypt_and_tag(mbedtls_gcm_context *ctx,
                              int mode,
                              size_t length,
	...
	switch(ctx->keysize){
			...
			case 128:
				...
				memcpy((uint32_t *) key, ctx->keyval,16);
				/*
    ...
    unsigned char* plaintext, unsigned int plaintext_len, unsigned char* aad, unsigned int aad_len, unsigned char* tag) {
				...
				AES_128_GCM_ENCRYPT(key, iv, iv_len,output,&out_len,input, length, add,add_len,tag);
			...
			case 192:
				key = malloc(24 * sizeof(unsigned char));
				memcpy((uint32_t *) key, ctx->keyval,24);

				AES_192_GCM_ENCRYPT(key, iv, iv_len,output,&out_len,input, length, add,add_len,tag);
			...
			case 256:
				...
				memcpy((uint32_t *) key, ctx->keyval,32);

				AES_256_GCM_ENCRYPT(key, iv, iv_len,output,&out_len,input, length, add,add_len,tag);
                              ...
                              unsigned char *tag)
{
	if(ctx->cipher == MBEDTLS_CIPHER_ID_AES)
		return custom_gcm_crypt_and_tag(ctx,mode,length,iv,iv_len,add,add_len,input,output,tag_len,tag);
...
int custom_gcm_auth_decrypt(mbedtls_gcm_context *ctx,
                             size_t length,
                             const unsigned char *iv,
                             size_t iv_len,
                             const unsigned char *add,
		...
		switch(ctx->keysize){
				...
				case 128:
					...
					*/
					...
					AES_128_GCM_DECRYPT(key, iv, iv_len,input,length,output, &out_len, add,add_len,tag,&res);
				...
				case 192:
					...
					AES_192_GCM_DECRYPT(key, iv, iv_len,input,length,output, &out_len, add,add_len,tag,&res);
				...
				case 256:
					key = malloc(32 * sizeof(unsigned char));
					memcpy((uint32_t *) key, ctx->keyval,32);
					//aes_256_gcm_decrypt_hw(key, iv, iv_len, input, length, output, &out_len, add, add_len, tag, &res, 0);

					AES_256_GCM_DECRYPT(key, iv, iv_len,input,length,output, &out_len, add,add_len,tag,&res);
                             ...
                             unsigned char *output)
{
	...
	if(ctx->cipher == MBEDTLS_CIPHER_ID_AES)
			return custom_gcm_auth_decrypt(ctx,length,iv,iv_len,add,add_len,tag,tag_len,input,output);


	else if ((ret = mbedtls_gcm_crypt_and_tag(ctx, MBEDTLS_GCM_DECRYPT, length,
