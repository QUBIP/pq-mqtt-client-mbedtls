...
int mbedtls_ssl_handshake(mbedtls_ssl_context *ssl)
{
    ...
    while (ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER) {
        ret = mbedtls_ssl_handshake_step(ssl); // il nono passaggio e' davvero troppo lungo
...
unsigned char mbedtls_ssl_sig_from_pk(mbedtls_pk_context *pk)
{
    ...
    if (mbedtls_pk_can_do(pk,MBEDTLS_PK_ED25519_MLDSA44)) {
       	return MBEDTLS_SSL_SIG_HPQ;
    }
#endif
...
} tls_id_curve_name_table[] =
{
	...
	{ MBEDTLS_SSL_IANA_TLS_GROUP_MLKEM768 , "x25519+Kyber768"},
    { 0, NULL },
};
