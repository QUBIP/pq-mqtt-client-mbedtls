...
int qubip_generate_and_write_key_exchange(
	...
	HybridKeyKEM * keypair = hybrid_key_gen();

	*out_len = keypair->mlkem_768_pk_size + keypair->x25519_pk_size;
	...
	ret = mbedtls_ecp_gen_key(MBEDTLS_ECP_DP_CURVE25519, &ecp, mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE);

	mbedtls_mpi_write_binary_le(&ecp.d, keypair->x25519_sk, 32);
	mbedtls_mpi_write_binary_le(&ecp.Q.X,keypair->x25519_pk,32);

#ifdef SWAP_ORDER
	memcpy(buf, keypair->mlkem_768_pk,keypair->mlkem_768_pk_size);
	memcpy(buf + keypair->mlkem_768_pk_size , keypair->x25519_pk,keypair->x25519_pk_size);
#else
	memcpy(buf, keypair->x25519_pk,keypair->x25519_pk_size);
                                         ...
                                         size_t *out_len)
{
    ...
    *out_len = 0;
    ...
    if (group_id == MBEDTLS_SSL_IANA_TLS_GROUP_MLKEM768) { 
    	...
    	ret = qubip_generate_and_write_key_exchange(ssl, group_id, p, end, &key_exchange_len);
                                         ...
                                         const unsigned char *end)
{
    ...
    if (mbedtls_ssl_tls13_named_group_is_ecdhe(group) ||
    	...
    	group == MBEDTLS_SSL_IANA_TLS_GROUP_MLKEM768) {
