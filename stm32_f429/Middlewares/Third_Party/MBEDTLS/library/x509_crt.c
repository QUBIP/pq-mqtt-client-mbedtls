...
#endif /* MBEDTLS_PK_HAVE_ECC_KEYS */
	if(pk_alg == MBEDTLS_PK_ED25519_MLDSA65 || pk_alg == MBEDTLS_PK_ED25519_MLDSA44){
		return 0;
	}
                                    ...
                                    mbedtls_x509_crt_restart_ctx *rs_ctx)
{
    size_t hash_len;
    size_t hash_size;
    unsigned char* hash; //[MBEDTLS_MD_MAX_SIZE]; it must be dynamic cause we do not hash in ed22519
    int ret;
#if !defined(MBEDTLS_USE_PSA_CRYPTO)
    const mbedtls_md_info_t *md_info;
    ...
    if(hash_alg == MBEDTLS_MD_EDDSA){
    	...
    	hash = malloc(MBEDTLS_MD_MAX_SIZE);
    	hash_size = MBEDTLS_MD_MAX_SIZE;
    }
