...
static int pk_group_id_from_group(const mbedtls_ecp_group *grp,
	...
	cleanup: mbedtls_ecp_group_free(&ref);
...
static int pk_use_ecparams_rfc8410(const mbedtls_asn1_buf *params,
		mbedtls_ecp_group_id grp_id, mbedtls_pk_context *pk) {
 ...
 */
static int pk_parse_key_rfc8410_der(mbedtls_pk_context *pk, unsigned char *key,
		size_t keylen, const unsigned char *end,
		int (*f_rng)(void*, unsigned char*, size_t), void *p_rng) {
	int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
	size_t len;

	if ((ret = mbedtls_asn1_get_tag(&key, (key + keylen), &len,
	MBEDTLS_ASN1_OCTET_STRING)) != 0) {
		return MBEDTLS_ERROR_ADD(MBEDTLS_ERR_PK_KEY_INVALID_FORMAT, ret);
	}
	 ...
	 * unconditionally. */
	if ((ret = mbedtls_pk_ecc_set_pubkey_from_prv(pk, key, len, f_rng, p_rng))
			!= 0) {
		return ret;
	}
...
#if defined(MBEDTLS_RSA_C)
	if (pk_alg == MBEDTLS_PK_RSA) {
		ret = mbedtls_rsa_parse_pubkey(mbedtls_pk_rsa(*pk), *p,
				(size_t) (end - *p));
		if (ret == 0) {
...
static int pk_parse_key_sec1_der(mbedtls_pk_context *pk,
	...
	if (p != end) {
		...
		MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_ASN1_CONSTRUCTED | 0)) == 0) {
			if ((ret = pk_get_ecparams(&p, p + len, &params)) != 0 || (ret =
					pk_use_ecparams(&params, pk)) != 0) {
				return ret;
			}
	...
	if (p != end) {
		...
		MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_ASN1_CONSTRUCTED | 1)) == 0) {
			...
			if ((ret = mbedtls_pk_ecc_set_pubkey(pk, p, (size_t) (end2 - p)))
					== 0) {
				pubkey_done = 1;
			} else {
				/*
	...
	if (!pubkey_done) {
		if ((ret = mbedtls_pk_ecc_set_pubkey_from_prv(pk, d, d_len, f_rng,
				p_rng)) != 0) {
			return ret;
		}
	}
...
int qubip_parse_hybrid_private_key(mbedtls_pk_context *pk, unsigned char *p,
		unsigned char *end) {
	if (pk->pk_info->type != MBEDTLS_PK_ED25519_MLDSA65
			&& pk->pk_info->type != MBEDTLS_PK_ED25519_MLDSA44) {
		return 1;
	}
	...
	memcpy(ctx->mldsa_pri_key, p, ctx->mldsa_prisize);
	...
	memcpy(ctx->ed_pri_key, p, ctx->ed_prisize);
...
int qubip_parse_hybrid_public_key(mbedtls_pk_context *pk, unsigned char **p,
	...
	//MLDSA44
	if (pk->pk_info->type == MBEDTLS_PK_ED25519_MLDSA44) {
		mbedtls_ed25519_mlds44_ctx *ctx = pk->pk_ctx;
		//NOW WE HARDCODE EVERYTHING SINCE THE OFFSETS ARE ALWAYS THE SAME, WE CAN THEN MOVE TO ASN1 PARSING
	...
	//MLDSA65
		...
		mbedtls_ed25519_mlds65_ctx *ctx = pk->pk_ctx;
		//NOW WE HARDCODE EVERYTHING SINCE THE OFFSETS ARE ALWAYS THE SAME, WE CAN THEN MOVE TO ASN1 PARSING
...
static int pk_parse_key_pkcs8_unencrypted_der(mbedtls_pk_context *pk,
	...
	if (pk_alg == MBEDTLS_PK_ECKEY || pk_alg == MBEDTLS_PK_ECKEY_DH
		...
		if (MBEDTLS_PK_IS_RFC8410_GROUP_ID(ec_grp_id)
			...
			if ((ret = pk_use_ecparams_rfc8410(&params, ec_grp_id, pk)) != 0
					|| (ret = pk_parse_key_rfc8410_der(pk, p, len, end, f_rng,
							p_rng)) != 0) {
				mbedtls_pk_free(pk);
				return ret;
			}
...
#endif
		{
			if ((ret = pk_use_ecparams(&params, pk)) != 0 || (ret =
					pk_parse_key_sec1_der(pk, p, len, f_rng, p_rng)) != 0) {
				mbedtls_pk_free(pk);
				return ret;
			}
		}
	...
	if (pk_alg == MBEDTLS_PK_ED25519_MLDSA65
			|| pk_alg == MBEDTLS_PK_ED25519_MLDSA44) {
		qubip_parse_hybrid_private_key(pk, p, end);
	} else {
		return MBEDTLS_ERR_PK_UNKNOWN_PK_ALG;
	}
	end = p + len;
...
MBEDTLS_STATIC_TESTABLE int mbedtls_pk_parse_key_pkcs8_encrypted_der(
		mbedtls_pk_context *pk, unsigned char *key, size_t keylen,
	...
	if (mbedtls_oid_get_pkcs12_pbe_alg(&pbe_alg_oid, &md_alg, &cipher_alg)
			== 0) {
		if ((ret = mbedtls_pkcs12_pbe_ext(&pbe_params,
		MBEDTLS_PKCS12_PBE_DECRYPT, cipher_alg, md_alg, pwd, pwdlen, p, len,
				buf, len, &outlen)) != 0) {
	...
	if (MBEDTLS_OID_CMP(MBEDTLS_OID_PKCS5_PBES2, &pbe_alg_oid) == 0) {
		if ((ret = mbedtls_pkcs5_pbes2_ext(&pbe_params, MBEDTLS_PKCS5_DECRYPT,
...
int mbedtls_pk_parse_key(mbedtls_pk_context *pk, const unsigned char *key,
	...
	if (key[keylen - 1] != '\0') {
		...
		PEM_BEGIN_PRIVATE_KEY_RSA, PEM_END_PRIVATE_KEY_RSA, key, pwd, pwdlen,
				&len);
	}
	...
	if (ret == 0) {
		...
		if ((ret = mbedtls_pk_setup(pk, pk_info)) != 0
				|| (ret = mbedtls_rsa_parse_key(mbedtls_pk_rsa(*pk), pem.buf,
						pem.buflen)) != 0) {
			mbedtls_pk_free(pk);
		}
	...
	if (key[keylen - 1] != '\0') {
		...
		PEM_END_PRIVATE_KEY_EC, key, pwd, pwdlen, &len);
	}
	if (ret == 0) {
		pk_info = mbedtls_pk_info_from_type(MBEDTLS_PK_ECKEY);

		if ((ret = mbedtls_pk_setup(pk, pk_info)) != 0
				|| (ret = pk_parse_key_sec1_der(pk, pem.buf, pem.buflen, f_rng,
						p_rng)) != 0) {
			mbedtls_pk_free(pk);
		}
	...
	if (key[keylen - 1] != '\0') {
		...
		ret = mbedtls_pem_read_buffer(&pem,
		PEM_BEGIN_PRIVATE_KEY_PKCS8, PEM_END_PRIVATE_KEY_PKCS8, key, NULL, 0,
				&len);
	}
	if (ret == 0) {
	...
	if (ret == 0) {
		if ((ret = mbedtls_pk_parse_key_pkcs8_encrypted_der(pk, pem.buf,
				pem.buflen, pwd, pwdlen, f_rng, p_rng)) != 0) {
			mbedtls_pk_free(pk);
		}
...
int mbedtls_pk_parse_public_key(mbedtls_pk_context *ctx,
	...
	if (key[keylen - 1] != '\0') {
		...
		ret = mbedtls_pem_read_buffer(&pem,
		PEM_BEGIN_PUBLIC_KEY_RSA, PEM_END_PUBLIC_KEY_RSA, key, NULL, 0, &len);
	}
	...
	if (ret == 0) {
		...
		if ((ret = mbedtls_rsa_parse_pubkey(mbedtls_pk_rsa(*ctx), p, pem.buflen))
				!= 0) {
			mbedtls_pk_free(ctx);
		}
	...
	if (key[keylen - 1] != '\0') {
		...
		ret = mbedtls_pem_read_buffer(&pem,
		PEM_BEGIN_PUBLIC_KEY, PEM_END_PUBLIC_KEY, key, NULL, 0, &len);
	}
