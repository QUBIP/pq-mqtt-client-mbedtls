                              ...
                              size_t *hash_length)
{
    ...
    *hash_length = 0;
    if (!PSA_ALG_IS_HASH(alg) && !(alg == MBEDTLS_MD_EDDSA)) {
        return PSA_ERROR_INVALID_ARGUMENT;
    }
    ...
    switch (alg) {
        ...
        case PSA_ALG_FFDH:
            return mbedtls_psa_ffdh_key_agreement(attributes,
                                                  ...
                                                  shared_secret_size,
                                                  shared_secret_length);
        case PSA_ALG_PQ_ECDH_MLKEM:
        	return qubip_pq_x25519_mlkem768_key_agreement(peer_key,
                    peer_key_length,
                    key_buffer,
                    key_buffer_size,
                    shared_secret,
...
static psa_status_t psa_key_agreement_raw_internal(psa_algorithm_t alg,
                                                   ...
                                                   size_t *shared_secret_length)
{
    if (!(PSA_ALG_IS_RAW_KEY_AGREEMENT(alg) || alg == PSA_ALG_PQ_ECDH_MLKEM)) {
        return PSA_ERROR_NOT_SUPPORTED;
    }
